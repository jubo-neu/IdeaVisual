...
from ldm.others import read_pickle, pose_inverse
...
class AnswererTrainData(Dataset):
    def __init__(self, target_dir, input_dir, uid_set_pkl, image_size=256):
        ...
        print('TrainDatalength %d' % len(self.uids))
...
class AnswererEvalData(Dataset):
    def __init__(self, image_dir):
        ...
        for fn in Path(image_dir).iterdir():
            if fn.suffix=='.png':
                self.fns.append(fn)
        print('EvalDataLength %d' % len(self.fns))
...
class AnswererDataset(pl.LightningDataModule):
    ...
    def setup(self, stage):
        if stage in ['fit']:
            self.train_dataset = AnswererTrainData(self.target_dir, self.input_dir, uid_set_pkl=self.uid_set_pkl, image_size=256)
            self.val_dataset = AnswererEvalData(image_dir=self.validation_dir)
        else:
